...
def softmax_regression_using_pca(temp_parameter=1):
...
print("softmax_pca test_error=", softmax_regression_using_pca(temp_parameter=1))
...
# Reconstructing and ploting the first two images from 18 dim PCS model
...
X_centered, feature_means = center_data(train_x)
...
def softmax_regression_using_pca10(temp_parameter=1):
    theta, cost_function_history = softmax_regression(train_cube, train_y, temp_parameter, alpha=0.3, lambda_factor=1e-4, k=10, num_iterations=150)
    plot_cost_function_over_time(cost_function_history)
    test_error = compute_test_error(test_cube, test_y, theta, temp_parameter=1)

    return test_error

print("softmax_pca10 test_error=", softmax_regression_using_pca10(temp_parameter=1))
